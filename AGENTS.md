...
| 2026-09-16 | Pipeline automation (ADR-0018): auto-materialize the replay path; the submit edge stays a **determinism boundary** (silver remains a pure replay of bronze). Cost sits on the API credential as defense-in-depth, never in the graph | A real run landed 4 bronze rows while `silver_visual_annotations` stayed at 0 — lineage was correct but nothing acted on it, so the chain stopped at bronze. The manual-submit guard existed for cost; with cost protected at the key, that guard has no purpose left. The wall is NOT a cost measure — it keeps `silver_*` a deterministic function of bronze (ADR-0011) |
