"""HTTP client for the standalone inference (jobs) service.

Thin sync-httpx client against the inference service contract:
...
The service (~/repos/jobs service) does ALL media handling server-side,
...
DEFAULT_JOBS_SERVICE_URL = "http://127.0.0.1:8462"

_TERMINAL_STATES = frozenset({"completed", "failed"})

_SERVICE_REMEDY = (
    "start it: uv run qwen-batch in ~/repos/jobs service, "
...
def _resolve_base_url(base_url: str | None) -> str:
    ...
    return os.environ.get("JOBS_SERVICE_URL", DEFAULT_JOBS_SERVICE_URL).rstrip("/")


def _service_unavailable(base_url: str, detail: str) -> BatchServiceError:
    return BatchServiceError(
        f"inference service at {base_url} is unreachable ({detail}) — {_SERVICE_REMEDY}",
        base_url=base_url,
    )
...
def check_health(
    ...
    """GET /health. Raises :class:`BatchServiceError` loudly when down."""
    import httpx
