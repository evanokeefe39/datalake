...
Mocks the Apify client functions to avoid real API calls.
"""
...
def mock_apify_success():
    ...
    def _mock_stream_dataset(dataset_id, dest, *, token):
        lines = [
            {
                "id": "1",
                "shortCode": "abc",
    ...
    patches = [
        patch(
            "datalake.defs.instagram.assets.trigger_run",
            return_value=_FakeRunInfo("run_1", "ds_1"),
        ),
        patch("datalake.defs.instagram.assets.poll_run",
              return_value="ds_1"),
        patch(
            "datalake.defs.instagram.assets.stream_dataset",
            side_effect=_mock_stream_dataset,
        ),
    ]
    for p in patches:
        p.start()
...
def mock_apify_failed():
    ...
    patches = [
        ...
        patch(
            "datalake.defs.instagram.assets.poll_run",
...
class TestBronzeAsset:
    """ig_posts_raw under various conditions."""

    def test_successful_scrape(self, mock_apify_success, tmp_path):
        ...
        with patch("datalake.defs.instagram.assets.BRONZE_LAKE", tmp_path):
            with patch(
                "datalake.defs.instagram.assets.bronze_path",
            ) as mbp:
    ...
    def test_idempotent_rerun(self, mock_apify_success, tmp_path):
    ...
    def test_empty_dataset(self, mock_apify_empty, tmp_path):
        ...
        with patch("datalake.defs.instagram.assets.BRONZE_LAKE", tmp_path):
            with patch(
                ...
                assert len(result) == 0
                assert dest.exists()
                assert len(pl.read_parquet(dest)) == 0

    def test_apify_failure_raises(self, mock_apify_failed, tmp_path):
    ...
    def test_apify_timeout_raises(self, mock_apify_timeout, tmp_path):
