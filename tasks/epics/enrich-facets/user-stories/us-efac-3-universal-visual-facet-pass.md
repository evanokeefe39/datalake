...
- **Relates to:** E-ENRICH-SUMMARIES (ONE visual submit fans out at harvest to
  `gold_visual_annotations` AND `gold_visual_summaries` — same request, not two
...
- **Backend note (2026-09-10, reconciled to the settled contract):** media
  ...
  Output lands in `gold_visual_annotations`; schema + additive semantics
  unchanged.

## Story
**As a** pipeline operator, **I want** one universal visual call per post that
extracts the visual-necessary facets into `gold_visual_annotations`
(own prompt_hash + metadata, schema_versioned), **so that** cross-tab analytics
work without touching the gold schema and without re-running the multimodal
...
- AC1: One visual pass per post (media resolved once — native images + ffmpeg
      ...
      `gold_visual_annotations` AND `gold_visual_summaries` (US-ESUM-1) — never
      one submit per table.
- AC2: Output additive — the new `gold_visual_annotations` table keyed
      `(post_id, domain)` with its own `prompt_hash`; existing gold tables and
...
- AC6: The visual pass records ITS OWN provenance (`prompt_hash` + `model` +
      `schema_version` + `run_id`) on `gold_visual_annotations` — per-pass
      provenance (epic audit P0-4); the text pass
      (`gold_text_annotations`) may not overwrite it.
...
- [ ] Schema catalog + readiness green for `gold_visual_annotations`.
- [ ] Scoped tests pass; no regression to the gold path.
...
- Adding a facet to the schema re-runs only the cheap text call
      (`gold_text_annotations`), never the visual input (regression guard on
