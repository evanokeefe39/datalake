...
- **Relates to:** E-ENRICH-TRANSCRIPTS (transcript channel:
  `silver_audio_transcripts`), E-ENRICH-ENGINE
- **Settled contract (2026-09-10):** the text pass lands its verbatim
  response in `bronze_enrichment_raw` and writes
  `silver_text_annotations`; ONE text submit fans out at conform to
  `silver_text_annotations` AND `silver_text_summaries` (US-ESUM-3). The DAG
  declares `silver_text_annotations` as DEPENDENT on `silver_audio_transcripts`.
...
`silver_text_annotations` in a separate, cheap TEXT call — **so that** schema
...
- AC1: Text facet call is media-free (caption + transcript + on-screen OCR only);
      ...
      `silver_text_annotations` keyed `(post_id, platform)` with its own
      `prompt_hash`.
- AC2: The expensive universal visual call's output is a fixed, rarely-changing
      *visual contract* (`silver_visual_annotations`); new semantic facets go to
...
- AC4: Transcript-optional — degrades gracefully to caption-only when a post
      ...
      `silver_audio_transcripts.transcript_status` (US-ETR-2): `done` → include
...
- AC5: The text pass records ITS OWN provenance (text `prompt_hash` + `model` +
      `schema_version` + `run_id`) on `silver_text_annotations` — per-pass
      provenance (epic audit P0-4); it must not touch the visual pass's
      provenance on `silver_visual_annotations`.
