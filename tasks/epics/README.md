...
| [`enrich-engine`](enrich-engine/epic.md) | E-ENRICH-ENGINE | Async enrichment engine (submit/harvest) on the standalone qwen-batch service (ADR-0009; was Gemini batch); **one seam, one bronze landing (ADR-0012)** | Active (pivot landed: ADR-0009/0012 implemented; tables live) | dlc-worker | E-MEDIA | `tasks/plans/inference-service-seam`, `dagster-native-orchestration-implementation`, `qwen-batch-enrichment`, `docs/architecture/adr/0009` + `0012`, US-EENG-1/2/3/4 |
...
| [`enrich-facets`](enrich-facets/epic.md) | E-ENRICH-FACETS | **NEW** cross-modal growth facets (V3) + engagement-utility | Active (silver tables wired; visual passes populated, text/audio passes not yet run) | dlc-worker | E-ENRICH-ENGINE, E-MEDIA, E-SERVING-ANALYTICS | `docs/architecture/pipelines/enrichment.md` (v3 canonical), `tasks/plans/silver-conform-tables`, `facet-list-experiment-design`, US-EFAC-1..4 |
| [`enrich-summaries`](enrich-summaries/epic.md) | E-ENRICH-SUMMARIES | **NEW** content_summary + per-image carousel summaries (folded call) | Open (design) | dlc-worker | E-ENRICH-ENGINE, E-MEDIA | `docs/architecture/pipelines/enrichment.md` (v3 canonical), `tasks/plans/silver-conform-tables`, US-ESUM-1/3 |
| [`enrich-transcripts`](enrich-transcripts/epic.md) | E-ENRICH-TRANSCRIPTS | **NEW** ffmpeg audio-extract → faster-whisper, incremental + backfill | Open (design) | dlc-worker | E-MEDIA | `docs/architecture/pipelines/enrichment.md` (v3 canonical), `tasks/plans/silver-conform-tables`, US-ETR-1/2/3 |
...
| [`creator-discovery`](creator-discovery/epic.md) | E-DISCOVERY | **NEW** cost-governed creator discovery & tiering: audience-band discovery across niches, `suggested_core` (warehouse) vs core allocation (ops depth), owner-allocated via script (admin view deferred), reported monthly Apify cost | Active (US-DISC-7/8 implemented 2026-09-17, PR #89; US-DISC-1..4 open — 5 absorbed by 7, 6 superseded by 8) | dlc-worker / sdlc-worker | E-IDENTITY, E-ENRICH-LABELS, E-SERVING-ANALYTICS | `tasks/plans/post-performance-observations-investigation.md` (cost model), ADR-0017 (roster boundary), US-DISC-1..8, `cross-check-2026-09-17.md` |
